export class AdminProductAPI{
    ...
    async create(newProduct) {
        try {
            const token = localStorage.getItem("authToken");
            const response = await fetch('http://localhost:8080/Heladiva/api/product/admin/save', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    "Authorization": `Bearer ${token}`
    ...
    async update(UpdatedProduct) {
        try {
            const token = localStorage.getItem("authToken");
            const response = await fetch('http://localhost:8080/Heladiva/api/product/admin/update', {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                    "Authorization": `Bearer ${token}`
    ...
    async delete(productID) {
        try {
            ...
            const response = await fetch(`http://localhost:8080/Heladiva/api/product/admin/delete/${productID}`, {
                method: "DELETE",
                headers: headers,
            });
